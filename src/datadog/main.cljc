(ns datadog.main
  "Kotodama WASM entrypoint for the Datadog clean-room actor (L4) — Clojure port.
  ...
  facts (`datadog.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "datadog")
(def tier "L4")
...
(def entity-specs
  [{:entity "Monitor"    :plural "monitors"     :id-prefix "datadog_mon"
    :fields [:name :type :query :message :overallState :tags] :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "Dashboard"  :plural "dashboards"   :id-prefix "datadog_das"
    :fields [:title :layoutType :description :widgets]        :required [:title :layoutType]
    :coerce {}                       :refs {}}
   {:entity "TimeSeries" :plural "timeserieses" :id-prefix "datadog_tim"
    :fields [:metric :type :timestamp :value :host :tags]     :required [:metric :type]
    :coerce {:timestamp :int :value :float} :refs {}}
   {:entity "Event"      :plural "events"       :id-prefix "datadog_eve"
    :fields [:title :text :timestamp :priority :alertType :tags] :required [:title :text]
    :coerce {:timestamp :int}        :refs {}}
   {:entity "Downtime"   :plural "downtimes"    :id-prefix "datadog_dow"
    :fields [:monitorId :scope :start :end :timezone]         :required [:scope :start]
    :coerce {:monitorId :int :start :int :end :int} :refs {:monitorId "Monitor"}}
   {:entity "Incident"   :plural "incidents"    :id-prefix "datadog_inc"
    :fields [:title :severity :status :customerImpactScope]   :required [:title :severity]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"datadog.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "datadog-compat" :tier tier :entities entities} 200])
...
(defn start! [] :datadog-compat/ready)
